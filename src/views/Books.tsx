...
import { Plus, Search, Edit2, Trash2, Calendar, Target, Clock, BookOpen, Filter, ArrowUpDown, CheckCircle2, Circle, AlertCircle, Info, StickyNote } from 'lucide-react';
...
const BookCard = React.memo(({ 
  ...
  return (
    <motion.div 
      ...
      className="card p-6 space-y-4 hover:shadow-2xl transition-all group relative overflow-hidden flex flex-col"
    >
...
export default function Books({ books, tasks, settings, onRefresh }: { books: Book[], tasks: any[], settings: AppSettings, onRefresh: () => void }) {
  ...
  const sortedAndFilteredBooks = useMemo(() => {
    ...
    return result;
  }, [books, searchTerm, filterBranch, filterStatus, filterForecast, sortBy, sortOrder, daysUntilExam]);
